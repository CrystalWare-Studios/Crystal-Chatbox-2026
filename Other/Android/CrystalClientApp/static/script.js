(() => {
    ...
    ].filter((part) => !IS_QUEST || !["window", "system_stats"].includes(part.key));
    ...
    function bindAppearance() {
        onClick("theme_toggle", async () => {
            const settings = getSettings();
            const next = settings.theme === "light" ? "dark" : "light";
            await saveSettingsWithToast({ theme: next }, "Theme updated.");
        });
        onClick("streamer_toggle", async () => {
            const settings = getSettings();
            await saveSettingsWithToast({ streamer_mode: !settings.streamer_mode }, "Streamer mode updated.");
        });
        onClick("compact_toggle", async () => {
            const settings = getSettings();
            await saveSettingsWithToast({ compact_mode: !settings.compact_mode }, "Compact mode updated.");
        });
        onClick("slim_toggle", async () => {
            const settings = getSettings();
            await saveSettingsWithToast({ slim_chatbox: !settings.slim_chatbox }, "Slim chatbox updated.");
        });
    ...
    async function toggleMessagePart(setting, checked) {
        ...
        try {
            if (setting === "system_stats_enabled") {
                ...
                await saveSettings({ show_vr_battery: checked });
                ...
                cards: [
                    {
                        ...
                        help: "Requires Spotify authorization and internet access.",
    ...
    function renderVrBatteryPanel(battery, settings) {
        ...
        if ($("vr_battery_show_chatbox") && document.activeElement !== $("vr_battery_show_chatbox")) {
            $("vr_battery_show_chatbox").checked = !!settings.show_vr_battery;
        }
    ...
    async function saveVrBatterySettings() {
        const showChatbox = $("vr_battery_show_chatbox") ? $("vr_battery_show_chatbox").checked : false;
