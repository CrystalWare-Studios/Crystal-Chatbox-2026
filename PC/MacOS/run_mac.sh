...
TRACKER_DIR="$PLATFORM_DIR/osc_body_tracker"
...
if [ ! -d "$TRACKER_DIR" ]; then
  echo "Tracker folder not found: $TRACKER_DIR"
  exit 1
fi
...
if [ -d "$TRACKER_VENV" ] && [ ! -x "$TRACKER_VENV/bin/python3" ] && [ ! -x "$TRACKER_VENV/bin/python" ]; then
  rm -rf "$TRACKER_VENV"
fi

if [ ! -d "$TRACKER_VENV" ]; then
  "$PYTHON_BIN" -m venv "$TRACKER_VENV"
fi

TRACKER_PYTHON="$TRACKER_VENV/bin/python3"
if [ ! -x "$TRACKER_PYTHON" ]; then
  TRACKER_PYTHON="$TRACKER_VENV/bin/python"
fi

"$TRACKER_PYTHON" -m pip install --upgrade pip
"$TRACKER_PYTHON" -m pip install -r "$TRACKER_DIR/requirements.txt" pillow

echo ""
echo "Starting Crystal Client for macOS on http://127.0.0.1:${PORT:-5000}"
echo "Open the Body Tracking tab to start/stop FBT."
echo ""
