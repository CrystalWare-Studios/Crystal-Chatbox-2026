...
def create_app():
    ...
    def vr_battery_settings():
        body = request.get_json(silent=True) or {}
        SETTINGS["show_vr_battery"] = bool(body.get("show_vr_battery", SETTINGS.get("show_vr_battery", False)))
    ...
    def save_layout():
        ...
        allowed = {"time", "custom", "vrchat_live", "song", "window", "heartrate", "weather", "system_stats", "vr_battery", "afk", "mute"}
    ...
    def spotify_auth():
        try:
            return_to = f"{request.host_url}spotify-callback"
            auth_url = f"https://crystalwarestudios.com/spotify/login?{urlencode({'return_to': return_to})}"
    ...
    def spotify_callback():
        try:
            ...
            if refresh_token:
                ...
                SETTINGS["spotify_redirect_uri"] = "https://crystalwarestudios.com/spotify/callback"
